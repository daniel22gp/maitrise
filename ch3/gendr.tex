...
On récupère les lexicalisations de surface de chacune des unités lexicales avec les règles \emph{lex\_class} (qui s'occupe des lexèmes comme \lex{Paul}) et \emph{lex\_lu}, ce qui est illustré à la figure~\ref{fig:lexsurf}. Il y en a deux, car il faut une règle spécifique pour les lexèmes appartenant à des classes puisque leurs traits de surface ne sont pas encodés dans leurs entrées lexicales, mais dans la classe qui leur est assignée.
...
Également, l'une des raisons qui nous a poussé à nous tourner vers une ressource lexicale verbale provient des limites du logicel MATE quant à l'encodage de multiples patrons de régime pour une même entrée \footnote{Pour bien présenter le problème, il nous aurait fallu présenter les fonctionnements de MATE en détails dès le début, mais comme c'est très complexe, la lecture des chapitres \ref{ch:python} et \ref{ch:implementation} clarifiera ce problème.}. Par exemple, pour un verbe donné, on ne peut pas avoir deux parties du discours différentes compétitionnant pour la même position syntaxique. Autrement dit, si nous voulions exhaustivement représenter les comportements du verbe \lex{want}, il nous faudrait un \ac{GP} qui puisse tenir compte du fait que le second actant syntaxique de ce verbe peut avoir une \ac{DPOS} de type verbal ou nominal: \form{I want to eat} vs \form{I want a dog}. Cela nous était impossible à encoder dans MATE avec les paramètres que nous avions car le système ne nous laissait pas donner deux versions de l'actant syntaxique II de \lex{want}. Ce qui s'offrait à nous comme solution inélégante était de créer deux verbes \lex{want} qui encoderaient séparément ces comportements syntaxiques. Cela n'était pas viable car nous aurions peuplé inutilement notre dictionnaire d'entrées verbales quasi identiques se distinguant uniquement par leur régime. Ce n'était pas une solution attrayante, car il nous paraissait logique que les patrons de régime d'un verbe se trouvent dans l'entrée de celui-ci, et non de créer une entrée verbale par patron de régime (ex: \lex{want} avec objet direct nominal et \lex{want} avec objet direct verbal). C'est donc inélégant d'un point de vue linguistique et pratique, car pour générer du texte il faudrait préciser dans nos données d'input à quel \lex{want} on fait référence. Cela devient rapidement désagréable lorsqu'un verbe contient des patrons de régimes multiples.
