...
\draft{expliquer pourquoi FrameNet aurait saturé notre dictionnaire d'information, et pourquoi le fonctionnemenent en classe verbale est mieux}
...
\draft{En théorie Sens-Texte, la relation prédicat-argument est représentée en étiquettant le noeud sémantique qui lie un prédicat à son argument. Puisqu'un prédicat peut posséder plusieurs arguments, sa relation de dépendance avec ceux-ci doit pouvoir se distinguer. Une numérotation des arcs est donc effectuée, mais tel que \cite[pp.~227--234]{mel2012semantics} le précise, les chiffres sur ces arcs ne sont pas porteurs de sens car ils n'aident qu'à distinguer les différents arguments d'un prédicat. C'est donc ce système que nous utilisons pour remplacer l'information donnée par les rôles thématiques. On la remplace par le concept \emph{d'actants sémantiques}, ce qui provient de la théorie Sens-Texte. Ils ont été mis de côté car \cite{mel2012semantics} jugeait que les rôles thématiques portaient trop de failles.}
\FL{ok, mieux lisser le texte}
