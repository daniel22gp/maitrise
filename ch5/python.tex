...
\draft{Les dictionnaires que nous planifions pour GenDR: semanticon, lexicon et gpcon
Les fichiers de type dictionnairiques que nous généront avec Python: lexicon.dict(contient la hiérarhcie de VerbNet et les classes verbales), members.dict(contient les membres, donc les unités lexicales avec leur classe verbale correspondante), gpcon. Parmi ceux-ci, lexicon et members de Python seront combinés pour former la majorité du prochain lexicon de GenDR. Il sera complété par le lexique qu'il possédait déjà et certaines classes abstraites déjà créées. La dynamique des dictionnaires de GenDR sera expliquée au chapitre suivant, référence au chapitre.}
