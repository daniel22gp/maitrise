...
\draft{refaire ce paragraphe}Après avoir fait filtrer le tout, nous avons donc procédé à la création du dictionnaire de patron de régime. Nous en sommes venus à la conclusion que nous ne pouvions pas directement extraire les propriétés lexicales des \acp{GP} de VerbNet, bien que c'était l'objectif initial. Comme nous l'avons vu plus tôt, un patron de régime englobe x, mais les cadres syntaxiques de VerbNet ne nous donnait pas complètement ce X. Il nous fournissait Y %les rôles thématiques employés par VerbNet pour décrire les actants syntaxiques ne nous fourni pas d'information pertinente en \ac{TST}. Toutefois, les constructions de VerbNet nous donnait des informations importantes comme la restriction sur la partie du discours des actants sélectionnés par le verbe, l'ordre de surface des actants syntaxiques par rapport au verbe et les les prépositions imposées par certains verbes pour réaliser les objets indirects ou obliques.
...
Expliquer brièvement comment on a procédé:\draft{définir les objets, définir une fonction pour prendre les objets et leur attribuer une étiquette syntaxique, puis créer les régimes grâce à ça}
