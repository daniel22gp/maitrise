...
Nos critères d'évaluation étaient les suivants: le rappel et la précision. Concrètement, nous avons évalué le rappel ainsi: \[\frac{\text{nb str attendues qu'on peut générer}}{\text{nb str attendues}}\]. Nous avons ensuite traduit cette fraction en pourcentage. Puis, nous avons noté la précision en la représentant ainsi:\[\frac{\text{nb str correctes}}{\text{nb str générées}}\]. Nous avons aussi converti cette fraction en pourcentage.
...
Erreurs de rappel imprévisibles: le verbe n'est pas encodé par VerbNet. Par le fait même ses patrons de régime n'y seront pas ce qui mènera à un échec très clairement (I beg her for the do). Mécanisme d'héritage deffectueux (breathed fire). Erreurs d'encodage dans le dictionnaire (oublié de mettre qqch dans le lexicon). un autre cas d'erreur dans le dictio. troisieme erreur dans le dictio. pas l'entrée believe, manque la diathèse de donate, oubli d'une préposition. mécanisme d'héritage, mauvaises désambiguisation dans l'input. Les classes par défaut fonctionnnet mal ont des problèmes (la quote, les locin dans les places, locin de proper noun, ). 
...
Les erreurs humaines aussi poseront problèmes: On avait le gp grill\_1 au lieu de celui de grill\_3 ce qui a généré 4 phrases mauvaises sémantiquement. Ça génère très très peu de structures correctes sémantiquement, mais bcp de structures syntaxiques.

Le cas de believe qui n'est pas dans le dictionnaire et le système ne sait pas faire quoi avec. Il réalise un arbre en apparence correct, mais qui ne veut rien dire car il croit que belive est un nom. le cas de backed out of going aussi. tous les cas de I begged her for the do,etc.

\draft{Analyser une dernière fois les résultats en regardant la précision pour voir ce qui pose problème}
