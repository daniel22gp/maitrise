...
Les erreurs de précisions sont dues à quelques facteurs. Les erreurs humaines mentionnées précédemment. Par exemple, (si on met la mauvaise désambiguisation, les gps qui viennent avec vont générer des phrases, mais qui ne veulent rien dire: ex grill\_1: She grilled the steak on me, she grilled the steak in me , etc.)

Erreur de VerbNet: Quand c'est un verbe qui n'existe pas dans le répetoire de VerbNet. Le cas de believe qui n'est pas dans le dictionnaire et le système ne sait pas faire quoi avec. Les règles de lexicalisation fallback. Lorsqu'on ne sait pas quoi faire avec un sémantème, on le lexicalise comme un nom. Ce qui donne un arbre fini, mais agrammatical: He converted the believe. Il réalise un arbre en apparence correct, mais qui ne veut rien dire car il croit que belive est un nom. tous les cas de I begged her for the do,etc.

Pour un bon gp, deux prépositions possibles (une bonne, une mauvaise)Un cas classique qui est encodé dans plusieurs entrées de patron de régime est le cas où deux prépositions sont listées par VerbNet pour le même actant syntaxique. Par exemple, dans l'évaluation on a vu x. Et la sélection de la préposition y lors de la réalisation de surface rendait la phrase agrammaticale. quand deux prépositions compétitionnent pour le même actant syntaxique, le système génère les 2. C'est un problème qui nous est hérité de VerbNet et que nous n'avions pas prévu puisque nous pensions que VerbNet avait prévu des scénarios où deux prépositions compétitionnent pour la même place, mais qu'ils ne seront jamais parfaitement interchangeables. The doctor cured pat of pneumonia et the docteur cured pat out of pneumonia.

Pour un bonne classe verbale, on récupère tous ses traits gps. Si l'un d'entre eux fonctionnent avec la diathèse mais que sa réalisation rend la phrase agrammaticale, c'est une erreur de précision. Une bonne partie du noise vient du fait que quand on réalise les gps, tous ceux d'une classe sont réalisés en surface. Le système prend le noeud dominant, il va cherhcher son équivalent dans le lexicon. celui-ci pointe vers une classe. Cette classe a des gps. Puis la règle sélection gp s'applique et pour chaque gp sélectionné, le système crée un nouvel arbre. Seuls ceux qui ont une diathèse qui correspond à la diathèse demandée en input seront réalisés encore plus loin et ceux qui ont un gp qui correspond à la diathèse d'une partie de l'input (teacher talked about history). Cela fait en sorte qu'un gp avec une diathèse matchant l'input sémantique sera réalisé, mais il est possible que cet output ne veuille rien dire. C'est le cas du choix du gp Barry Cryer erased at the writing, au lieu de Erased the writing. Donc, il peut fort probablement y avoir une incompatibilité sémantique entre l'input et le gp, ce qui fait qu'il sera généré correctement en syntaxe, mais que la phrase est agrammaticale pour des raisons sémantiques (). Par exemple \form{The street gushed}

\subsection{Synthèse de l'évaluation}

La mesure F1 qui est calculée en  de 69%

Ce qu'on retire de tout cela sont deux problèmes majeurs. Le mécanisme d'héritage qui fonctionne partiellement avec l'architecture présente qui nuit au rappel. Tel que nous l'avons mentionné, nous pouvons résoudre à court terme le problème d'héritage en implémentant les gps hérités directement dans les sous-classes. Il faudrait se doter d'une mécanisme de filtrage pour rejeter les structures incomplètes.
