...
Au chapitre~\ref{chapgendr}, nous avons montré comment le lexique s'encodait dans la version initiale de GenDR. Nous avions deux dictionnaires: un dictionnaire de sémantèmes (\emph{semanticon}) et un dictionnaire de lexèmes (\emph{lexicon}). Toutefois, comme nous venons de l'exposer, la nouvelle version de GenDR sera complétée par l'ajout d'un troisième dictionnaire: le \emph{gpcon}, un dictionnaire de patron de régime. Ce dictionnaire de \ac{GP} était une conséquence de l'intégration de VerbNet à notre système.

Pour l'implémentation de ces nouvelles données lexicales dans GenDR, il y avait plusieurs avenues possibles. Nous aurions pu intégrer le tout dans le \emph{lexicon} directement, mais il semblait logique de stocker les patrons de régime dans un autre fichier puisqu'il s'agit de données lexicales de nature différente. De plus, en intégrant les patrons de régime dans le \emph{lexicon} nous l'aurions surpeupler puisqu'un patron de régime $X$ est probablement utilisé par plusieurs classes verbales, donc il aurait été répété de multiples fois inutilement. Ainsi, de créer le \emph{gpcon} à part nous permettait d'éviter d'être redondant dans l'organisation du lexique.

Dans la version initiale de GenDR \citep{LambreyImplementationcollocationspour2017,lareau18}, les patrons de régime étaient listés dans le \emph{lexicon} même puisqu'il n'y en avait pas énormément. Mais comme nous voulons intégrer 278 patrons de régime, il faut revoir la composition de ce dictionnaire et l'intéraction entre les règles de correspondances sémantiques et les dictionnaires. Toutes nos règles de grammaires étaient codées en fonction de l'emplacement des \acp{GP} dans le \emph{lexicon}. C'est pourquoi nous avons dû refaire une partie de notre \emph{lexicon} et nous avons dû ajouter et modifier des règles sémantiques de notre grammaire pour adpater le système à la venue de VerbNet.
