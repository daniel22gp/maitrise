...
\draft{Nous avons créé des scripts Python qui nous ont aidé à extraire les phrases exemples de VerbNet, puis à les encoder dans des fichiers .str qui seront lus par GenDR. les graphes sémantiques qui serviront d'inputs à nos tests.}
...
\draft{Petite précision sur les gps du nom qui ne fonctionnent pas tjs bien: Pour Linda winked her eye, problème de GenDR, j’en ai hérité. Les erreurs qu’on compte, ce sont les erreurs des GPS de verbe. Le attendu en vert, le généré en jaune dans ce cas-ci. Y’a une erreur, mais c’est pas une erreur de gps des verbe. Les gps des noms. Les erreurs de GPS de noms, pas du tout le focus de mon travail, on les a noté dans l’annexe, mais on ne les a pas compilé dans les erreurs.}

\draft{Il faut changer le paragraphe pour: les silences dûs à VerbNet, vu qu'il y a aussi le believing in qui n'a pas fonctionné. -	Mais c’Est pcq cette phrase est sous convert, mais le verbe believe n’est même pas dans les sous-verbe de convert -	Donc,believe in est impossible à générer
}
...
\draft{Nous n'avons pas comptabiliser dans nos erreurs les défauts liés à l'impossibilité de générer le phénomène langagier qu'est la manière de faire les complément du nom. C'est un problème de GenDR dont nous avons hérité, mais qui n'était pas ce que nous cherchions à tester. Les erreurs qu’on compte, ce sont les erreurs des GPS de verbe. Il s'agit d'erreurs liées aux GPs des noms, pas du tout le focus de mon travail. on les a noté dans l’annexe, mais on ne les a pas compilé dans les erreurs. Dans l'annexe~\ref{ch:annexe}, nous avons présenté les erreurs de GenDR en termes des GPs des noms}
